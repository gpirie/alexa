# Single Stream Audio Skill (Wave FM)
...
3. ```./models/*.json```
   ...
   Change the model defintion to replace the invocation name (it defaults to "wave fm") and the sample phrases for each intent.  
...
```bash
 $ ask simulate -l en-GB -t "alexa, play wave fm"
